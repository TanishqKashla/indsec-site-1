...
import {
  ...
  getCorporateDisclosureSection,
} from "../lib/corporateDisclosureSections";

/**
 * Corporate Disclosure — a single downloadable PDF shown on the grouped
 * /disclosures/corporate-disclosure page. Each document belongs to one section
 * (General Meeting Notices / Annual Returns / Governance Policies) and the page
 * renders one heading per section automatically.
 *
 ...
 */
export const corporateDisclosureType = defineType({
  ...
  fields: [
    defineField({
      ...
      options: {
        list: CORPORATE_DISCLOSURE_SECTIONS.map((s) => ({
          title: s.label,
          value: s.key,
        })),
        layout: "dropdown",
  ...
  preview: {
    ...
    prepare({ title, section, order }) {
      const label = getCorporateDisclosureSection(section)?.label ?? section;
      const orderText =
